...
describe CardConnect::Service::SettlementStatusResponse do
  ...
  it 'should have the batch id' do
    expect(@response.batchid).must_equal '71742042'
  end

  it 'should have the settlement status' do
    expect(@response.hoststat).must_equal 'GB'
  end

  it 'should have the host batch id' do
    expect(@response.hostbatch).must_equal '71742041'
  end
  ...
  it 'should have the transactions' do
    expect(@response.txns).must_equal [{ setlstat: 'N', retref: '179001161341' },
                               { setlstat: 'Y', retref: '179002161341' }]
  end

  describe '#body' do
    it 'should have all the right fields in the body' do
      expect(@response.body.keys).must_equal [:merchid, :batchid, :hoststat, :hostbatch, :respproc, :txns]
    end
    ...
    it 'should generate hash with all the right values' do
      ...
      expect(@response.body).must_equal valid_payload
    end
  end
end
