...
describe CardConnect::Service::SettlementStatusRequest do
  ...
  describe 'FIELDS' do
    ...
    it 'should have retrieval reference number' do
      expect(@request.date).must_equal '0110'
    end
  end

  describe '#valid?' do
    it 'should not be valid if no attributes are passed in' do
      expect(CardConnect::Service::SettlementStatusRequest.new.valid?).must_equal false
    end

    describe 'should be valid if valid attributes are passed in' do
      it 'is valid if date is passed' do
        expect(CardConnect::Service::SettlementStatusRequest.new(valid_settlestat_request_date).valid?).must_equal true
      end

      it 'is valid if batchid is passed' do
        expect(CardConnect::Service::SettlementStatusRequest.new(valid_settlestat_request_batch).valid?).must_equal true
      end
    end
  end
  ...
  describe '#errors' do
    CardConnect::Service::SettlementStatusRequest::REQUIRED_FIELDS.each do |field|
      ...
      it "should have an error message if #{field_name} is missing" do
        expect(CardConnect::Service::SettlementStatusRequest.new.errors).must_include "#{field_name} is missing"
      end
    end
    ...
    describe '#validate_date_format' do
      it 'should have an error when date is less than 4 characters long' do
        ...
        expect(request.errors).must_include 'Date format is invalid. Please use MMDD format'
      end
  ...
  describe '#payload' do
    describe 'when date is passed' do
      it 'should generate the correct path params' do
        expect(@request.payload).must_equal "?merchid=#{@request.merchid}&date=#{@request.date}&"
      end
    end
    ...
    describe 'when batchid is passed' do
      it 'should generate the correct path params' do
        ...
        expect(request.payload).must_equal "?merchid=#{@request.merchid}&batchid=#{request.batchid}&"
      end
    end
  end
end
