...
describe CardConnect::Service::ProfileGetRequest do
  ...
  describe '#valid?' do
    it 'should not be valid if no attributes are passed in' do
      expect(CardConnect::Service::ProfileGetRequest.new.valid?).must_equal false
    end

    it 'should be valid if valid attributes are passed in' do
      expect(CardConnect::Service::ProfileGetRequest.new(valid_profile_request).valid?).must_equal true
    end
  end
  ...
  describe '#errors' do
    CardConnect::Service::ProfileGetRequest::REQUIRED_FIELDS.each do |field|
      ...
      it "should have an error message if #{field_name} is missing" do
        expect(CardConnect::Service::ProfileGetRequest.new.errors).must_include "#{field_name} is missing"
      end
    end
  end
