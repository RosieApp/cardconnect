...
describe CardConnect::Service::CaptureRequest do
  ...
  describe 'FIELDS' do
    ...
    it 'should have retrieval reference number' do
      expect(@request.retref).must_equal '288002073633'
    end

    it 'should have authorization code' do
      expect(@request.authcode).must_equal '046221'
    end

    it 'should have amount' do
      expect(@request.amount).must_equal '596.00'
    end

    it 'should have invoice id' do
      expect(@request.invoiceid).must_equal '7890'
    end

    it 'should have PO number' do
      expect(@request.ponumber).must_equal 'PO-0736332'
    end

    it 'should have tax amount' do
      expect(@request.taxamnt).must_equal '40.00'
    end
  end

  describe '#valid?' do
    it 'should not be valid if no attributes are passed in' do
      expect(CardConnect::Service::CaptureRequest.new.valid?).must_equal false
    end
  ...
  describe '#payload' do
    it 'should generate hash with all the right values' do
      expect(@request.payload).must_equal symbolize_keys(valid_capture_request)
    end
  end
end
